...
export default function EventsPage() {
    ...
    return (
        <div>
            ...
            <div className="mt-4 md:mx-8 lg:mx-80">
                {events.length > 0 && events.map(event => (
                    <Link to={'/account/events/' + event._id} className=" flex cursor-pointer gap-4 bg-gray-100 p-4 rounded-2xl" key={event._id}>
                        <div className="flex w-32 h-32 bg-gray-200 grow shrink-0">
                            <EventsImg event={event}/>
                        </div>
